...
<%
	...
	String val = StringEscapeUtils.escapeHtml4(request.getParameter("name"));
		...
		<div id="content">
			...
			<div>
				<div class="introduction">
					<h1>
						Search for <span class="titleItem"><i><%=val%></i></span>
					</h1>
				</div>
						...
						<tbody>
							...
							for (SearchIndex si : indeces) {
								...
								case Term.TERM_GENE:
									objTypeName = "Gene";
									break;
								case Term.TERM_REGULATOR:
									objTypeName = "Regulator";
									break;
								case Term.TERM_REGULON:
									objTypeName = "Regulon";
									break;
								}
								if (objTypeName.equals("undef"))
									continue;
								i++;
								String regType = si.getRegulationTypeTermId() == Term.TERM_REGULATION_TYPE_TF ? "TF" : "RNA";
							%>
							<tr <%if (i % 2 == 0) {%> style="background: #EFE" <%}%>>
								<td><span class="<%=regType%>"> <%=regType%>
								</span></td>
								<td><%=objTypeName%></td>
								<td style="text-align: left"><%=si.getObjName()%></td>
								<td style="text-align: left"><a
									href="regulon.jsp?regulon_id=<%=si.getRegulonId()%>"><%=si.getRegulatorName()%>
										- <%=si.getGenomeName()%></a></td>
							</tr>
							<%
								}
							%>
